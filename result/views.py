...
def result(request, desk_id):
    ...
    vote_matrix, victories_matrix, winner_list = candidates_duals(votes_list, len(candidates_list))
    for candidate in winner_list:
        candidate[1] = "{} _ {}".format(candidates_list[candidate[1]].first_name.capitalize(), candidates_list[candidate[1]].last_name.upper())
...
def result_details(request, desk_id):
    ...
    for candidate in candidates_list:
        candidates_names_list.append("{} {}".format(candidate.first_name.capitalize(), candidate.last_name.upper()))
